...
import type { BranchUsageResult } from '@lukailun/dev-kit-claudecode/get-claude-code-branch-usage';
    ...
    try {
      threads = db
        .query<
          ThreadRow,
          [string, string]
        >('SELECT id, rollout_path, model FROM threads WHERE git_branch = ? AND cwd = ?')
        .all(branch, root);
    } finally {
      db.close();
    }
